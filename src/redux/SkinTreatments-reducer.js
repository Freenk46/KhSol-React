...
const SkinTreatmentsreducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    case UPDATE_PROCEDURE_NAMES: {
      for (var i = 0; i < state.SkinTreatments.length; i++) {
        if (i === action.ProcedureClassId) {
          ProcedureNames = state.SkinTreatments[i];
        }
      }
      let stateCopy = { ...state };
      stateCopy.SkinTreatments.ProcedureNames = ProcedureNames;
      return state
    }
